...
function c61665245.spfilter1(c,e,tp,zone)
	return c:IsRace(RACE_DRAGON+RACE_SPELLCASTER) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP_DEFENSE,1-tp,zone)
end
...
function c61665245.spfilter2(c,e,tp,lg)
	return c:IsRace(RACE_DRAGON+RACE_SPELLCASTER) and  c:IsFaceup() and lg:IsContains(c) and Duel.IsExistingMatchingCard(c61665245.spfilter3,tp,LOCATION_DECK,0,1,nil,e,tp,c:GetRace())
end
function c61665245.spfilter_chkc(c,e,tp,lg,rc)
	return  c:IsRace(RACE_DRAGON+RACE_SPELLCASTER) and c:IsFaceup() and lg:IsContains(c) and (c:GetRace()&rc)==rc 
end
function c61665245.spfilter3(c,e,tp,rac)
	if c:IsRace(RACE_DRAGON+RACE_SPELLCASTER) and not c:IsRace(rac) then return false end
	local ok=false
	for p=0,1 do
