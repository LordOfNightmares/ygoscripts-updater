...
function c58481572.hdtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then 
		return Duel.IsExistingMatchingCard(Card.IsAbleToRemove,tp,0,LOCATION_FIELD,1,nil)
		and Duel.IsExistingMatchingCard(Card.IsAbleToRemove,1-tp,0,LOCATION_FIELD,1,nil)
	end
	Duel.SetOperationInfo(0,CATEGORY_REMOVE,nil,1,tp,LOCATION_FIELD)
end
function c58481572.hdop(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(Card.IsAbleToRemove,tp,0,LOCATION_FIELD,nil)
	if g:GetCount()>0 then
		local sg=g:RandomSelect(tp,1)
		Duel.Remove(sg,POS_FACEUP,REASON_EFFECT)
	end
	local g=Duel.GetMatchingGroup(Card.IsAbleToRemove,1-tp,0,LOCATION_FIELD,nil)
	if g:GetCount()>0 then
