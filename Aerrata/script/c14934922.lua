...
function c14934922.filter(c)
	return c:IsFaceup() and c:IsSetCard(0x119) and c:IsSummonType(SUMMON_TYPE_LINK) and c:GetFlagEffect(14934922)~=0 and Duel.IsPlayerCanRelease(c:GetControler(),c)
end
function c14934922.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if op==0 then
		...
		local tc=Duel.SelectTarget(tp,c14934922.filter,tp,LOCATION_MZONE,0,1,1,nil)
...
function c14934922.activate(e,tp,eg,ep,ev,re,r,rp)
	if e:GetLabel()==0 then
		...
		if not Duel.Release(tc,REASON_EFFECT) then link=0 end
		local g=Duel.SelectMatchingCard(tp,aux.TRUE,tp,0,LOCATION_ONFIELD,1,link,nil)
